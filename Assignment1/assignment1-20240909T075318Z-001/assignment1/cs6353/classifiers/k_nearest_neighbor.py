...
class KNearestNeighbor(object):
  ...
  def compute_distances_two_loops(self, X):
    ...
    for i in range(num_test):
      for j in range(num_train):
        ...
        #####################################################################
        # pass
        # distance(x, y) = sqrt(sum((x[d] - y[d])^2))
  ...
  def compute_distances_no_loops(self, X):
    ...
    # distance(x, y) = sqrt(x^2 + y^2 - 2xy)
    X_square = np.sum(X ** 2, axis=1).reshape(num_test, 1)
    Y_square = np.sum(self.X_train ** 2, axis=1).reshape(1, num_train)
  ...
  def predict_labels(self, dists, k=1):
    ...
    for i in range(num_test):
      ...
      #########################################################################
      # pass
      closest_k_neighbours = np.argsort(dists[i])[:k]
      closest_y = self.y_train[closest_k_neighbours]
